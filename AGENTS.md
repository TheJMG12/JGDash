...
- Client opt-in: `auth.experimental.passkey: true` in `js/supabase-client.js`. **`registerPasskey()` requires an existing session** — enroll via email OTP/`verifyOtp` (or magic-link return) first, then register. `signInWithPasskey()` is for returning users.
